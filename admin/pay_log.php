...
if ($wdcp_gid==1) {
	$sum=sum_result("select * from wd_dns_paylog $wh");
	$query=$db->query("select * from wd_dns_paylog $wh order by id desc limit $start,$pagenum");
}else{
	$sum=sum_result("select * from wd_dns_paylog where uid='$wdcp_uid'");
	$query=$db->query("select * from wd_dns_paylog where uid='$wdcp_uid' order by id desc limit $start,$pagenum");
}
$list=array();
$i=0;
  while ($r=$db->fetch_array($query)) {
	...
	$list[$i]['id']=$r['id'];
	if (empty($r['out_trade_no']))
		$list[$i]['out_trade_no']="后台充值";
	else
		$list[$i]['out_trade_no']=$r['out_trade_no'];
	if ($wdcp_gid==1 and !empty($r['trade_no']))
		$list[$i]['trade_no']=" (".$r['trade_no'].")";		
	else
		$list[$i]['trade_no']="";;		
...
require_once(G_T("admin/pay_log.htm"));
?>