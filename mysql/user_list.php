...
if (isset($_POST['Submit_chgpw'])) {
	...
	if ($wdcp_gid==1)
		...
		$q=$db->query("select * from wd_mysql where uid='$wdcp_uid' and id='$id'");
	if ($db->num_rows($q)==0) go_back("数据库用户名不存在！");
...
if (isset($_POST['Submit'])) {
	...
	$wh="dbuser like '%$dbuser%' and";
	$whg="where dbuser like '%$dbuser%'";
